...
**Notes:**
- Every threshold alert above (except `sensorFault`) is confirmed over `SENSOR_TRANSIENT_FAILURE_THRESHOLD` consecutive *new sensor samples* (not loop ticks - a reading that hasn't refreshed yet doesn't advance the count) before it raises, and is Schmitt-trigger hysteresis-gated (Config.h's per-parameter `*_ALERT_HYSTERESIS` constants) so it clears only once the reading has recovered past `threshold +/- hysteresis`, not merely back across the same line - see `AlertManager::setAlertDebounced()`/`newSampleAvailable()`/`aboveWithHysteresis()`/`belowWithHysteresis()`. Together this makes each alert represent one confirmed abnormal episode with exactly one false->true notification edge, instead of re-arming on every noisy reading near the boundary. `sensorFault` keeps the older per-tick debounce (no hysteresis - it is a binary valid/invalid check, not a threshold).
