...
**pH and EC correction.** When a reading is out of its safe range and has been confirmed stable, the firmware picks a direction (pH Up or pH Down, or for EC, "raise" using the grow and bloom pumps or "dilute" by adding water through the solenoid), runs the matching pump for a fixed dose time, then circulates and watches quietly for 90 seconds (`PH_STABILIZATION_TIME` / `EC_STABILIZATION_TIME`) before the first check. After that, the firmware does not use a fixed retry count anymore. It gives the whole correction a 4 minute time budget from the moment it started (`PH_EC_CORRECTION_STALL_TIMEOUT_MS`), and within that budget it redoses either when a reading has held steady but is still out of range for 25 seconds straight (`PH_EC_STABLE_HOLD_FOR_PUBLISH_MS`), or sooner if a check every 30 seconds (`PH_EC_RECHECK_INTERVAL_MS`) shows the reading moving the wrong way. If the target still has not been reached once the 4 minute budget runs out, the subsystem locks and asks for manual attention. (The older `MAX_PH_ATTEMPTS` / `MAX_EC_ATTEMPTS` fixed-retry-count constants this paragraph used to name no longer exist in the code, replaced by this time budget.) A short cooldown (`PH_DOSE_COOLDOWN` / `EC_DOSE_COOLDOWN`, one minute) also has to pass since the last dose before a brand new correction can start, because a probe can look "stable" before the just-dosed chemical has actually finished mixing in.
...
**Dosing and correction timing** (`PH_DOSE_COOLDOWN`/`EC_DOSE_COOLDOWN`, `PH_STABILIZATION_TIME`/`EC_STABILIZATION_TIME`, `PH_DOSING_TIME`/`EC_DOSING_TIME`, `PH_EC_CORRECTION_STALL_TIMEOUT_MS`, `PH_EC_RECHECK_INTERVAL_MS`, `PH_EC_STABLE_HOLD_FOR_PUBLISH_MS`): how long each dose pulse runs, how long the system waits before trusting a post-dose reading, and the total time budget allowed before giving up and locking the subsystem. These directly affect how aggressively the system chases pH and EC targets, so they should be changed conservatively. (`MIXING_DURATION` is defined in `Config.h` but is not read anywhere in the current code, it is dead and can be ignored or removed.)
